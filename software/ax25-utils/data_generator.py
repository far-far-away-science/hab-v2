...
def calculateNewAx25DataFromOld(oldNumberOfOnes, byte2Encode):
    (newDataGiventLastBitWasZero, newDataNumberOfBitsA, newNumberOfOnesA) = calculateNewAx25DataFromOldImpl(0, oldNumberOfOnes, byte2Encode)
    (newDataGiventLastBitWasOne, newDataNumberOfBitsB, newNumberOfOnesB) = calculateNewAx25DataFromOldImpl(1, oldNumberOfOnes, byte2Encode)
    ...
    return (oldNumberOfOnes, byte2Encode, newDataGiventLastBitWasZero, newDataGiventLastBitWasOne, newDataNumberOfBitsA, newNumberOfOnesA)
...
def calculateNewAx25DataFromOldImpl(oldLastBit, oldNumberOfOnes, byte2Encode, performStuffing = True):
    ...
    for iBit in range(8):
        ...
        if bit != 0: # insert one bit
            if lastBit == 1: # no inversion needed as last bit was one
            ...
            if performStuffing and newNumberOfOnes == 5:
                if lastBit == 1: # we can add zero as is as last bit was one
                    lastBit = 0
                    newData &= ~(1 << (newDataNumberOfBits))
            ...
            newNumberOfOnes = 0
            newDataNumberOfBits += 1
    return (newData, newDataNumberOfBits, newNumberOfOnes)
...
def generateAllByteToAx25DataCombinations():
    ...
    for iNumberOfOnes in range(6):
        for iData2Encode in range(256):
            ...
            (oldNumberOfOnes, byte2Encode, newDataGiventLastBitWasZero, newDataGiventLastBitWasOne, newDataNumberOfBits, newNumberOfOnes) = data
